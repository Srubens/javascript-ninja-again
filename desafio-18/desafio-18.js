(function(){
	...
	*/
	...
	let otherCpf = ['049.860.030-03']

	otherCpf.forEach((index)=>{
		console.log( 'otherCpf ' + cleanCPF(index) )
	})
	...
	*/
	...
	otherCpf.forEach((index)=>{
		console.log('otherCpf ' + cleanCPF(index).replace(/(\d{3})(\d{3})(\d{3})(\d{2})/,'$1.$2.$3-$4'))
	})
	...
	*/
	...
	console.log( `<h1>O texto dentro da tag "h1" é "Título da página"</h1><p>O texto dentro da tag "p" é "Este é um parágrafo"</p><footer>O texto dentro da tag "footer" é "Rodapé"</footer>`.replace(
		/<(\w+)>([^<]+)<\/\w+>/gm, "<$1>O texto dentro da tag '$1' é '$2'</$1>\n") )
